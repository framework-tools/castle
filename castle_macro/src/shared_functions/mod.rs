...
use syn::{punctuated::Punctuated, token::Comma, Field, spanned::Spanned};
...
pub fn get_input_def_and_initalizations(punctuated : Punctuated<Field, Comma>) -> (Vec<TokenStream>, Vec<TokenStream>) {
    let (a, b) = punctuated
    .iter()
    .map(|field| {
