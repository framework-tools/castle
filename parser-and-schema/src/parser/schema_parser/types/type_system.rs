...
use crate::{tokenizer::tokenizer::{Tokenizer, advance_and_parse_token}, token::token::TokenKind, parser::schema_parser::parse_schema_field::skip_comma};
...
where R: Read{
    ...
    match token {
        Some(token) => match token.kind {
            ...
            TokenKind::Identifier(identifier) => return get_schema_type(identifier.name, cursor),
            TokenKind::VecType(vec_type) => return get_vec_type(vec_type, cursor),
...
fn get_primitive_type<R>(primitive_type: PrimitiveType, cursor: &mut Cursor<R>) -> Result<Type, CastleError> 
where R: Read{
    advance_and_parse_token(cursor)?; // skip comma 
    return Ok(Type::PrimitiveType(primitive_type))
}

fn get_schema_type<R>(identifier: Box<str>, cursor: &mut Cursor<R>) -> Result<Type, CastleError> 
where R: Read{
    advance_and_parse_token(cursor)?; // skip comma 
    return Ok(Type::SchemaTypeOrEnum(identifier))
}

fn get_vec_type<R>(vec_type: VecType, cursor: &mut Cursor<R>) -> Result<Type, CastleError>
    where R: Read{
    advance_and_parse_token(cursor)?; // skip comma 
