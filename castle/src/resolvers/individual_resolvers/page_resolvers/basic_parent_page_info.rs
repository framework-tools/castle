...
-> Result<Value<R>, CastleError> {
    ...
    fn dummy_data_for_basic_parent_page_info<R>() -> Value<R>{
        //dummy data
        let (_, dummy_data): (HashSet<Box<str>>, Value<R>) = create_possible_fields_and_dummy_data(vec![
            ("title".into(), Value::String("Parent Page Title".to_string())),
            ("icon".into(), Value::String("".to_string())), 
            ("emoji".into(), Value::String("😊".to_string())),
        ]);
        return dummy_data
    }
    let dummy_data = dummy_data_for_basic_parent_page_info();
