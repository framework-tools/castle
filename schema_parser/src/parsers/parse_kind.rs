...
pub(crate) fn parse_generics(tokenizer: &mut impl Tokenizable) -> Result<Vec<Box<str>>, CastleError> {
            ...
            loop {
                ...
                if token.kind == TokenKind::Punctuator(Punctuator::GenericClose) {
                    if generics.is_empty() {
                        return Err(CastleError::Schema("provide type with no generics", token.span));
                    }
                    tokenizer.expect_punctuator(Punctuator::GreaterThan, true)?;
                    break Ok(generics);
                }
