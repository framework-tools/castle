    ...
    loop {
        ...
        match kind {
            TokenKind::Keyword(Keyword::Type) => {
                ...
                schema_definition.types.insert(type_.identifier.clone(), type_);
            },
            ...
            TokenKind::Keyword(Keyword::Directive) => {
                if directives.len() != 0 {
                    return Err(CastleError::Other("Directive definitions cannot have directives.".into()));
                }
                let directives = parse_directive_definition(&mut tokenizer)?;
                schema_definition.directives.insert(directives.name.clone(), directives);
            },
            _ => return Err(CastleError::Schema(format!("Expected item, found: {:?}", kind).into(), span))
        }
    }
}